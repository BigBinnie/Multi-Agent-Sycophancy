...
for trigger_num_1 in {0..8}; do
    for trigger_num_2 in {0..8}; do
        for trigger_num_3 in {0..8}; do
            if [ "$trigger_num_2" -ge "$trigger_num_1" ] && [ "$trigger_num_3" -ge "$trigger_num_2" ]; then
...
bash scripts/run_batch_multi_agent_3_solvers_trigger.sh \$trigger_num_1 \$trigger_num_2 \$trigger_num_3
"
